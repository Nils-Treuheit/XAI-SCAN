...
# python eval.py --config_exp configs/selflabel/selflabel_cifar20.yml --model results/cifar-20/selflabel/model.pth.tar
...
def main():
    ...
    config['topk_neighbors_val_path'] = "results/cifar-20/pretext/topk-val-neighbors.npy"
    #print(config)
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        # # Predicted clusters
        # print(predictions[0]["predictions"][1:10])
        
        # # Target clusters
        # print(predictions[0]["targets"][1:10])

        # print("Compare shapes: \n")
        # print(predictions[0]["predictions"].shape)
        # print(predictions[0]["targets"].shape)
        
        #print("Features Shape:",features.size())
        if config['topk_neighbors_val_path'] is None:
        ...
        print("Prototypes: ",prototype_indices)
        print("Prototypes probs shape: \n",predictions[0]["probabilities"][prototype_indices].shape)
        # visualzie images with same "prediction"
        
        # visualize_explanations(prototype_indices,dataset,
        #                        predictions[0]["probabilities"][prototype_indices],
        #                        config,dataloader, model)
        
        # dataset - anchor - neighbor
        index_of_image = prototype_indices[0]
        chosen_anchor = dataset[index_of_image]['anchor']
        cluster = dataset[index_of_image]["target"]
        possible_neighbour = dataset[index_of_image]["neighbor"]
        
        print("Anchor:",chosen_anchor.size())
        print("Neighbour:", possible_neighbour.size())
        
        import numpy as np
        import matplotlib.pyplot as plt

        image = np.transpose(chosen_anchor, (1, 2, 0))
        ...
        print("Dataset anchor: ", dataset[0]['anchor'].size())
        print("Dataset neighbor: ", dataset[0]['neighbor'].size())
        print("Dataset possible_neighbors: ", dataset[0]['possible_neighbors'].size())
        print("Dataset cluster: ", dataset[0]['target'])

        # batch predict is - get_predictions(config, dataloader, model, return_features=True)
        # np.array(pill_transf(img)) - next(iter(dataloader))
        if args.visualize_prototypes:
            prototype_indices = get_prototypes(config, predictions[head], features, model)
            #images =visualize_indices(prototype_indices, dataset,clustering_stats['hungarian_match'])
            images = visualize_indices(prototype_indices,predictions[0]["predictions"] ,dataset)
            return images
    else:
        raise NotImplementedError
...
def visualize_explanations(indices,dataset, probabilities, config, dataloader, model):
    import matplotlib.pyplot as plt
    import numpy as np
    for idx in zip(indices):
        ...
        explanation = explainer.explain_instance(np.array(img), 
                                         get_predictions(config, dataloader, model, return_features=True), # classification function
                                         top_labels=5, 
                                         hide_color=0, 
                                         num_samples=1000)
...
def visualize_indices(indices, dataset, hungarian_match):
    import matplotlib.pyplot as plt
    import numpy as np
    print("Hungarian match:")
    print(hungarian_match)
    images = []
    for idx, (index, class_idx) in zip(indices, hungarian_match):
        ...
        plt.figure()
        plt.axis('off')
        plt.title(dataset.classes[class_idx])
        plt.imshow(img)
        plt.show()
