...
@torch.no_grad()
def get_prototypes(config, predictions, features, model, topk=20):
...
def visualize_cluster(images_for_cluster_idx,dataset, cluster, show_images):
    ...
    for idx in  tqdm(images_for_cluster_idx, total =len(images_for_cluster_idx), desc = "Getting captions..." ):
        image = dataset.get_image(idx)['anchor']
...
def get_clustering_stats(predictions, head):
        ...
        clustering_stats = hungarian_evaluate(head, predictions,all_clusters_str, 
...
def show_wordcloud(captions):
    ...
    word_cloud = WordCloud(collocations = False, background_color = 'white').generate(single_string) 
