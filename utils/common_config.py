...
def get_criterion(p):
    if p["criterion"] == "simclr":
        ...
        criterion = ConfidenceBasedCE(
            p["confidence_threshold"], p["criterion_kwargs"]["apply_class_balancing"]
        )
...
def get_model(p, pretrain_path=None):
    ...
    if pretrain_path is not None and os.path.exists(pretrain_path):
        ...
        if (
            ...
            p["setup"] == "selflabel"
        ):  # Weights are supposed to be transfered from scan
...
def get_train_dataset(
    p, transform, to_augmented_dataset=False, to_neighbors_dataset=False, split=None
):
    # Base dataset
...
def get_val_dataloader(p, dataset) -> torch.utils.data.DataLoader:
    return torch.utils.data.DataLoader(
        dataset,
        num_workers=p["num_workers"],
        batch_size=p["batch_size"],
